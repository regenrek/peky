---
name: run-dev
description: Launch the peky dev workflow by running scripts/dev-run in the current repo. Use when the user says "run dev" or asks to open a fresh Ghostty dev session that go installs, restarts the daemon, and launches peky.
---
...
## Workflow
1) Ensure the current working directory is the peky repo root (look for `scripts/dev-run`).
