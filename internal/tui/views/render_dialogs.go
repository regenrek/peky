...
func (m Model) viewAuthPrompt() string {
	inputWidth := 50
	if m.Width > 0 {
		inputWidth = clamp(m.Width-30, 24, 80)
	}
	m.AuthPrompt.Input.Width = inputWidth
	title := strings.TrimSpace(m.AuthPrompt.Title)
	if title == "" {
		title = "Auth"
	}
	note := strings.TrimSpace(m.AuthPrompt.Note)
	inputLine := theme.DialogLabel.Render("Input: ") + m.AuthPrompt.Input.View()
	choices := renderDialogChoices([]dialogChoice{
		{Key: "enter", Label: "confirm"},
		{Key: "esc", Label: "cancel"},
	})
...
func (m Model) viewAuthProgress() string {
	title := strings.TrimSpace(m.AuthProgress.Title)
	if title == "" {
		title = "Auth"
	}
	body := strings.TrimSpace(m.AuthProgress.Body)
	if body == "" {
		body = "Working..."
	}
	footer := strings.TrimSpace(m.AuthProgress.Footer)
	if footer == "" {
		footer = "esc close"
	}
	content := dialogContent(
		dialogTitleStyle.Render(title),
		body,
		renderDialogChoices([]dialogChoice{{Key: "esc", Label: footer}}),
	)
	return m.renderDialog(dialogSpec{Content: content})
}
