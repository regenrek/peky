...
func (m *Model) viewModel() views.Model {
	...
	vm := views.Model{
		...
		AuthProviderPicker:       m.authProviderPicker,
		AuthMethodPicker:         m.authMethodPicker,
		AuthPrompt: views.AuthPrompt{
			Title: m.authPromptTitle,
			Note:  m.authPromptNote,
			Input: m.authPromptInput,
		},
		AuthProgress: views.AuthProgress{
			Title:  m.authProgressTitle,
			Body:   m.authProgressBody,
			Footer: m.authProgressFooter,
		},
